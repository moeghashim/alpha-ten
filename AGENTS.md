...
- `CURSOR_API_KEY`, `GITHUB_TOKEN`, `RENDER_API_KEY` available **only** for tasks that explicitly use them. Other tasks should be implemented without them and stubbed in tests if needed.
- Never log secrets. Never commit `.env`. `.env.example` is the only env file in git.
